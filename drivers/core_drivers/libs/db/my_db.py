...
class my_db(object):
    ...
    def connect(self):
        self.my_conn = MySQLdb.connect(
            host=self.server, user=self.username, passwd=self.password, db=self.db
        )
    ...
    def insert_order_hist(
        self, order_id, test_id, tes_result, tes_ref, tes_unit, tes_flag
    ):
        inst_name = "<?>"
        test_name = "<?>"
        q_data = self.my_select(
            " SELECT name FROM corelab_instruments WHERE id = '%s' "
            % self.instrument_id
        )
        if len(q_data) > 0:
            inst_name = q_data[0][0]
        q_data = self.my_select(
            " SELECT name FROM corelab_tests WHERE id = '%s' " % test_id
        )
        if len(q_data) > 0:
            test_name = q_data[0][0]

        act_txt = (
            "Result %s with ref.range[%s], unit[%s] and flag[%s] set for analyt %s "
            % (tes_result, tes_ref, tes_unit, tes_flag, test_name)
        )

        self.my_insert(
            " INSERT corelab_historyorders (action_code,action_user,action_text,action_date,lastmodification,order_id,test_id)\
    ...
    def get_test_array(self, sample_no, instrument_id):
        try:
            ...
            logging.debug(
                f"checking test order.id[{or_id}] for instrument.id{instrument_id}]..."
            )
            ...
            SQL = SQL.format(or_id=or_id, instrument_id=instrument_id)
            logging.debug(SQL)
    ...
    def set_status_batch(self, sample_no, status):
        try:
            ...
            for os_id in g_or_id:
                self.my_update(
                    " update corelab_instrumentbatch set status = '%s' where order_sample_id = '%s' "
                    % (status, os_id[0])
                )
            return True
        except Exception as e:
...
and ib.status = %s "
    ...
    def get_barcode_id(self, sample_no):
        try:
            data = self.my_select(
                " SELECT corelab_ordersamples.id,corelab_ordersamples.order_id,corelab_ordersamples.specimen_id,corelab_specimens.name \
                    FROM corelab_ordersamples \
    ...
    def get_patient_id(self, sample_no):
        """geting patient data based on sample_no"""
...
"""
    ...
    def get_order_id(self, sample_no):
        data = self.my_select(
            " SELECT order_id FROM corelab_ordersamples WHERE sample_no = '%s' "
            % sample_no
        )
        if len(data) > 0:
            return data[0][0]
    ...
    def create_pantologi_mark(self, result_id):
        ...
        data_res = self.my_select(
            " SELECT corelab_results.alfa_result,corelab_orderresults.ref_range,corelab_orderresults.id \
        ...
        # test_id
        data_res = self.my_select(
            "select test_id from corelab_results where id = " + str(result_id)
        )
        test_id = data_res[0][0]
        # gender_id
        data_res = self.my_select(
            "select corelab_patients.gender_id,corelab_patients.dob,corelab_orders.order_date from \
            corelab_results left join corelab_orders on corelab_results.order_id = corelab_orders.id \
            left join corelab_patients on corelab_orders.patient_id = corelab_patients.id  where corelab_results.id = "
            + str(result_id)
        )
        ...
        if not ref_range:
            ...
            ref_range = self.my_select(
                "select count(*) from corelab_testrefranges where any_age = 1 and gender_id is null and  test_id =  "
                + str(test_id)
            )
            have_data = False
            if ref_range[0][0] > 0:
                ...
                have_data = True
            ref_range = self.my_select(
                "select lower,upper,operator,operator_value,panic_lower,panic_upper from corelab_testrefranges where any_age = 1 and gender_id is null and  test_id =  "
                + str(test_id)
            )
            ...
            if have_data:
                ...
                tmp_ref_range = self.my_select(
                    "select count(*) from corelab_testrefranges where any_age = 0 and gender_id = "
                    + str(gender_id)
                    + " and  test_id =  "
                    + str(test_id)
                )
                ...
                if tmp_ref_range[0][0] > 0:
                    ...
                    have_data = True
                tmp_ref_range = self.my_select(
                    "select lower,upper,operator,operator_value,panic_lower,panic_upper,age_from,age_from_type,age_to,age_to_type from corelab_testrefranges where any_age = 0 and gender_id = "
                    + str(gender_id)
                    + " and  test_id =  "
                    + str(test_id)
                )
                if have_data:
                    ...
                    for ref_data in tmp_ref_range:
                        ...
                        if age_from_type == "D":
                            ...
                            if (
                                age_from
                                <= relativedelta(order_date, dob).days
                                <= age_to
                            ):
                                if not operator:
                                    ref_range_value = str(lower) + " - " + str(upper)
                                    ...
                                    ref_range_value = (
                                        str(operator) + " " + str(operator_value)
                                    )
        ...
        if self.is_float(alfa_res) and ref_range:
            ...
            # checking panic
            logging.info(
                "checking panic value [%s] [%s]" % (str(panic_lower), str(panic_upper))
            )
            if panic_lower:
            ...
            self.my_update(
                " UPDATE corelab_orderresults SET patologi_mark = '%s', ref_range = '%s'  WHERE id = '%s' "
                % (flag, ref_range, ordres_id)
            )

    def get_mapping_code_attribut(self, instrument_id, tes_code):
        data = self.my_select(
            " SELECT id,test_code,result_selection,sql_script,instrument_id,test_id FROM corelab_instrumenttests where instrument_id = %s and test_code = '%s' "
            % (instrument_id, tes_code)
        )
        if len(data) > 0:
            return True, data[0]
        else:
            logging.warning(
                "test_code [%s] for instrument_id [%s] not found."
                % (tes_code, instrument_id)
            )
            return False, 0
    ...
    def insert_result(
        ...
        if found:
            ...
            if sql_script != "None":
                try:
                    ...
                    logging.info(
                        "found SQL script for transform the result [%s]" % sql_script
                    )
                    logging.info("replace <test_result> with [%s]" % str(tes_result))
                    sql_script = str(sql_script).replace(
                        "<test_result>", str(tes_result)
                    )
                    ...
                    logging.error(
                        "Failed when executing SQL script [%s] error [%s]"
                        % (str(sql_script), str(e))
                    )
        ...
        if order_id > 0:
            ...
            mapp_inst = self.my_select(
                " SELECT test_id FROM corelab_instrumenttests WHERE instrument_id = '%s' AND test_code = '%s' "
                % (self.instrument_id, tes_code)
            )
            # check apakah receive flag and ref_range from instrument
            logging.info(
                "checking if instrument recive flag and ref range from instrument.."
            )
            res = self.my_select(
                " select receive_ref_ranges from corelab_instruments where id = "
            ...
            for m_test_id in mapp_inst:
                ...
                # 2. cek sampleid
                mapp_sample_id = self.my_select(
                    " SELECT id FROM corelab_ordersamples WHERE sample_no = '%s' "
                    % sample_no
                )
                ...
                for m_test_id in mapp_inst:
                    ...
                    data_tes = self.my_select(
                        " SELECT test_id FROM corelab_orderresults WHERE sample_id = '%s' and test_id = '%s' "
                        % (str(m_sample_id), str(m_test_id[0]))
                    )

                    if len(data_tes) > 0:
                        test_id = data_tes[0][0]
                        logging.info(
                            " Got mapped test_id [%s] for test_code [%s] with result [%s]"
                            % (test_id, tes_code, tes_result)
                        )
                        ...
                        if self.is_float(tes_result):
                            ...
                            try:
                                decimal_place = self.my_select(
                                    " SELECT decimal_place FROM corelab_testparameters WHERE test_id = '%s' "
                                    % (test_id)
                                )
                                if len(decimal_place) > 0:
                                    decimal_place = decimal_place[0][0]
                                    logging.info(
                                        "got decimal_place [%s] try to round from [%s] to [%s]"
                                        % (
                                            decimal_place,
                                            tes_result,
                                            round(
                                                float(str(tes_result)),
                                                int(str(decimal_place)),
                                            ),
                                        )
                                    )
                        ...
                        d_order_result = self.my_select(
                            " SELECT id FROM corelab_orderresults WHERE order_id = '%s' AND test_id = '%s' "
                            % (order_id, test_id)
                        )
                        if len(d_order_result) > 0:
                            ...
                            order_res_id = self.my_insert(
                                " INSERT INTO corelab_orderresults (order_id,test_id,is_header,lastmodification,validation_status,print_status) \
                                VALUES ('%s','%s','%s','%s','%s','%s') "
                        ...
                        if tes_flag != "":
                            ...
                            data_flag = self.my_select(
                                " SELECT id FROM corelab_instrumentflags WHERE flag_code = '%s' AND instrument_id = '%s' "
                                % (tes_flag, self.instrument_id)
                            )
                            ...
                            if len(data_flag) > 0:
                                ...
                                # insert with flag
                                logging.info(
                                    " insert result (alfa_result,instrument_id,order_id,test_id,flag_id,lastmodification) values ('%s','%s','%s','%s','%s','%s') "
                                    % (
                                        tes_result,
                                        self.instrument_id,
                                        order_id,
                                ...
                                last_id = self.my_insert(
                                    " INSERT INTO corelab_results (alfa_result,instrument_id,order_id,test_id,flag_id,lastmodification) VALUES ('%s','%s','%s','%s','%s','%s') "
                                    % (
                                        tes_result,
                                        self.instrument_id,
                                        order_id,
                                ...
                                # insert without flag
                                logging.info(
                                    " insert result (alfa_result,instrument_id,order_id,test_id,lastmodification) values ('%s','%s','%s','%s','%s') "
                                    % (
                                        tes_result,
                                        self.instrument_id,
                                        order_id,
                                ...
                                last_id = self.my_insert(
                                    " INSERT INTO corelab_results (alfa_result,instrument_id,order_id,test_id,lastmodification) VALUES ('%s','%s','%s','%s','%s') "
                                    % (
                                        tes_result,
                                        self.instrument_id,
                                        order_id,
                        ...
                        logging.info(
                            "checking ref range[%s] & flag[%s] " % (tes_ref, tes_flag)
                        )
                        logging.info(
                            "Receive ref range from instrument [%s]"
                            % str(receive_ref_range)
                        )
                        if (not tes_ref) and str(receive_ref_range) == "1":
                            logging.warning(
                                "have ref.renge from insrument but receive_ref_range value is 1. we skip it and generate automaticaly from db."
                            )
                            receive_ref_range = "0"
                        try:
                            b_fail = False
                            if str(receive_ref_range) == "0":
                                logging.info(
                                    " we not receive ref_ranges and flag from instrument "
                                )
                                self.my_update(
                                    " UPDATE corelab_orderresults SET result_id = '%s', \
                                ...
                                tes_ref == ""
                                and tes_flag != ""
                                and str(receive_ref_range) == "1"
                            ):
                                logging.info(
                                    " instrument not send reference range and flag for result."
                                )
                                self.my_update(
                                    " UPDATE corelab_orderresults SET result_id = '%s', unit = '%s',patologi_mark = '%s', \
                                    validation_status = '1', techval_user = NULL , techval_date = NULL, medval_user = NULL, medval_date = NULL WHERE order_id = '%s' AND test_id = '%s' "
                                ...
                                self.my_update(
                                    " UPDATE corelab_orderresults SET result_id = '%s', ref_range = '%s', unit = '%s', \
                                    validation_status = '1', techval_user = NULL , techval_date = NULL, medval_user = NULL, medval_date = NULL WHERE order_id = '%s' AND test_id = '%s' "
                                ...
                                tes_ref, tes_unit, tes_flag = self.costum_flag_range(
                                    tes_ref, tes_unit, tes_flag
                                )
                                self.my_update(
                                    " UPDATE corelab_orderresults SET result_id = '%s', unit = '%s', ref_range = '%s', patologi_mark = '%s' , \
                                    validation_status = '1', techval_user = NULL , techval_date = NULL, medval_user = NULL, medval_date = NULL WHERE order_id = '%s' AND test_id = '%s' "
                                    % (
                                        last_id,
                                        tes_unit,
                ...
                logging.warning(
                    " No mapped for sample_id [%s] with test_id [%s] at order"
                    % (m_sample_id, m_test_id)
                )
                return last_insert_id
